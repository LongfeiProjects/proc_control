...
namespace proc_control {
    ...
    PositionMode::EigenVector6d PositionMode::GetLocalError(Eigen::Vector3d &translation, Eigen::Vector3d &orientation, double dt) {
        ...
        if (fabs(orientation[2] - world_orientation_[2]) > 180) {
            local_error[YAW] = fmod(orientation[2] + (360 - world_orientation_[2]), 360.0);
            if (local_error[YAW] > 180) {
                local_error[YAW] -= 360;
            }
        } else {
            local_error[YAW] = orientation[2] - world_orientation_[2];
        }
